...
- 🎯 Entry points to execute your program using `python -m <fastapi_template>` or `$ fastapi_template` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/alessandroveras/fastapi-template/branch/main/graph/badge.svg?token=fastapi-template_token_here)](https://codecov.io/gh/alessandroveras/fastapi-template)
[![CI](https://github.com/alessandroveras/fastapi-template/actions/workflows/main.yml/badge.svg)](https://github.com/alessandroveras/fastapi-template/actions/workflows/main.yml)

Awesome fastapi_template created by alessandroveras

## Install

from source
```bash
git clone https://github.com/alessandroveras/fastapi-template fastapi_template
cd fastapi_template
make install
```
...
```bash
python -m fastapi_template run --port 8080
```

or

```bash
$ uvicorn fastapi_template:app
```
...
Run with `fastapi_template run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fastapi_template create-user`.
...
-----------------------------------------------------
fastapi_template/__init__.py              4      0   100%
fastapi_template/app.py                  16      1    94%
fastapi_template/cli.py                  21      0   100%
fastapi_template/config.py                5      0   100%
fastapi_template/db.py                   10      0   100%
fastapi_template/models/__init__.py       0      0   100%
fastapi_template/models/content.py       47      1    98%
fastapi_template/routes/__init__.py      11      0   100%
fastapi_template/routes/content.py       52     25    52%
fastapi_template/routes/security.py      15      1    93%
fastapi_template/routes/user.py          52     26    50%
fastapi_template/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fastapi_template/default.toml.

### As environment variables
```bash
export fastapi_template_KEY=value
export fastapi_template_KEY="@int 42"
export fastapi_template_KEY="@jinja {{ this.db.uri }}"
export fastapi_template_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fastapi_template_ENV=production fastapi_template run
```
